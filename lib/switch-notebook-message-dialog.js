...
const SwitchNotebookMessageDialog = (_) => {
  ...
  const open = () => {
    ...
    document.documentElement.style.setProperty(
      "--switch-notebook-menu-height",
      (height - 100).toString(10) + "px"
    );
    document.documentElement.style.setProperty(
      "--switch-notebook-dialog-margin-top",
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const ele = document.querySelector(".switch-notebook-dropdown input");
      if (ele != null) {
        ele.focus();
      }
    }, 100);
  ...
  return (
    <Dialog
      ...
      hiding={false}
      className="switch-notebook-dialog"
    >
      <Dialog.Content>
        <Dropdown
          className="switch-notebook-dropdown"
...
export default SwitchNotebookMessageDialog;
